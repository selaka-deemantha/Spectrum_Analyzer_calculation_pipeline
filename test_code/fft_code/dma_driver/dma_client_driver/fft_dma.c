...
#define DMA_BUF_SIZE (DMA_BUF_WORDS * sizeof(u64))  // 64-bit words
...
static ssize_t fft_dma_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    dma_cookie_t cookie;
    size_t words = min(count / sizeof(u64), (size_t)DMA_BUF_WORDS);

    reinit_completion(&priv->s2mm_cmp);
    dev_info(priv->dev, "[READ] Preparing DMA transfer for %zu 64-bit words\n", words);
...
MODULE_DESCRIPTION("FFT IP DMA driver using S2MM 64-bit channel");
